...
export async function GET(req: NextRequest) {
  ...
  const SQL =
    "Select * From BiSemana Where bisemana.bi_codigo In (Select itensnegocios.biSemana_bi_codigo from itensnegocios Where itensnegocios.Pontos_pon_codigo = ? And itensnegocios.Tipo In ('L','B','C','D','T','M') And itensnegocios.biSemana_bi_codigo is not null) And bisemana.bi_final >= CURDATE() Order By bi_inicio;";
